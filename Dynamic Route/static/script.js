...
function getTop10direct(){
  
  const country = window.location.pathname.substring(1);
  console.log("HERE", country)
  const countries = ['france', 'australia', 'brazil', 'hong-kong', 'india', 'japan','singapore','taiwan','united-kingdom','united-states']

  if (countries.includes(country)){
    getDetails(country);
  } else if (country != ''){
    console.log("HERE2", country)
    document.getElementById('error-message').innerHTML = "Country in URL not on list! Please select from list" 
  }
}
...
function pushCountry(){
    ...
    //console.log('gettop10', country)

    const country = document.getElementById("country").value;
    //console.log('gettop10 2', country)
    console.log(country)
